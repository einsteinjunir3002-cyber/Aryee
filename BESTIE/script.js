...
const reasonWords = ['Kind 🥺', 'Beautiful 🌸', 'Caring 🫂', 'Brilliant 🧠', 'Hilarious 😂', 'Loyal 🐕', 'Supportive 🤝', 'Fierce 🦁', 'Ambitious 🚀', 'Inspiring ✨'];
const reasonDescriptions = ['Your kindness makes every room warmer, seriously.', 'You carry beauty in everything you do, inside and out.', 'Your care is a gift to everyone lucky enough to know you.', 'Your mind is brilliant and endlessly fascinating.', 'Your laughter is my favorite soundtrack.', 'Your loyalty is rare, priceless, and deeply appreciated.', 'You lift people up with your steady, unwavering support.', 'You shine with a quiet, undeniable strength.', 'Your ambition and drive inspire us all.', 'You literally make the world feel brighter.'];
const messages = [
...
function isUnlockedByPassword() {
  return localStorage.getItem('unlockedByPassword') === '1';
}

function setUnlockedByPassword() {
  localStorage.setItem('unlockedByPassword', '1');
}
...
function initializeCountdown() {
  ...
  const interval = setInterval(() => {
    countdownEl.textContent = formatTimeLeft(unlockDate);
    if (new Date() >= unlockDate || isUnlockedByPassword()) {
...
async function loadAmbientMedia() {
  const mediaFiles = []; // No videos currently
...
function beginTyping() {
  const message = `Dear Josephine, 💖\n\nHappy Birthday! 🎂🥳\n\nToday is all about celebrating someone truly wonderful—YOU. ✨\nYou have a beautiful heart, a kind soul, and a smile that can instantly brighten anyone's day. ☀️\n\nThank you for every late-night conversation 🌙, every uncontrollable laugh 😂, every golden memory 📸, and for simply being the amazing, authentic person you are.\n\nI hope this new chapter of your life brings you endless happiness, radiant health, massive success, and exciting adventures. 🚀\n\nMay every dream in your heart blossom into reality. 🌻\n\nNever stop believing in yourself.\nNever stop shining your light. 🌟\n\nAlways remember that you are appreciated and loved far more than words can ever express. 🥰\n\nI hope today reminds you just how spectacularly special you truly are.\n\nHappy Birthday once again, Bestie! 🎉\n\nMay this be your happiest, most unforgettable year yet.\n\nWith all my love, ❤️`;
  let index = 0;
...
async function loadGallery() {
  const staticImages = ["./Pics/IMG-20200918-WA0021.jpg","./Pics/IMG-20200919-WA0009.jpg","./Pics/IMG-20200919-WA0010.jpg","./Pics/IMG_20200901_043248_547.jpg","./Pics/IMG_20200901_043248_550.jpg","./Pics/Snapchat-1068713668.jpg","./Pics/Snapchat-1372113039.jpg","./Pics/Snapchat-1524727519.jpg","./Pics/Snapchat-1754887728.jpg","./Pics/Snapchat-971677463.jpg"];
  images = shuffle(staticImages);
...
function renderSunflowers() {
  ...
  Array.from({ length: 12 }).forEach((_, index) => {
    ...
    sunflower.addEventListener('click', () => {
      // If already unlocked by password, just bloom normally
      ...
      const code = prompt('Enter password to unlock the birthday site');
      if (code === '1234') {
        setUnlockedByPassword();
        // reveal site immediately
        loadingScreen.classList.add('hidden');
        app.classList.remove('hidden');
        startExperience();
      } else {
        // shake feedback for wrong password
        sunflower.animate([
...
// Password lock bypass on the loading screen sunflower
const loadingSunflower = document.querySelector('.sunflower-bloom');
if (loadingSunflower) {
  loadingSunflower.style.cursor = 'pointer';
  loadingSunflower.addEventListener('click', () => {
    // Prevent multiple prompts if already unlocked
    if (isUnlockedByPassword() || new Date() >= unlockDate) return;
    
    const code = prompt('Enter secret password to unlock early:');
    if (code === '1234') {
      setUnlockedByPassword();
      loadingScreen.classList.add('hidden');
      app.classList.remove('hidden');
